...
ResultSheet = function() {
  ...
  this.addResultData = function() {
    ...
    gSheet
      .getRange("A2")
      .setFormula("query(" + g.NamedRange_Month + ',"select * ")');
  ...
  this.addSparkLineData = function() {
    ...
    for (var n = 1; n <= g.GameDateInfo.numOfWeek; n++) {
      //each week header
      gSheet
        .getRange(lastRow, 5 * n - 3, 1, 5)
        .setValues([gSparkHeader.slice(1)]); //except Name column at here

      //add sparkline data
      var indexForSparkLine = 5 * n - 3;
      for (i = indexForSparkLine; i <= indexForSparkLine + 3; i++) {
        gSheet
          .getRange(lastRow + 1, i, 1, 1)
          .setFormula(
            "iferror(value(vlookup($" +
              "A" +
              (lastRow + 1) +
              ", " +
              lookupRange +
    ...
    var tMember = players.map(function(elem) {
      return [elem];
    });
    ...
    gSheet
      .getRange(startRow, 2, 1, g.GameDateInfo.numOfWeek * 5)
      .copyTo(
        gSheet.getRange(
          startRow + 1,
          2,
          players.length - 1,
          g.GameDateInfo.numOfWeek * 5
        )
      );
  };
  ...
  this.setNamedRange = function() {
    ...
    app.setNamedRange(
      g.NamedRange_Result,
      gSheet.getRange(4, 1, players.length, lastCol)
    );

    app.setNamedRange(
      g.NamedRange_SparkLine,
      gSheet.getRange(sparkLineRow, 1, players.length, lastCol)
    );
  };
};
